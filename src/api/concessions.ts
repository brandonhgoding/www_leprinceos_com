...
export const concessionsApi = {
  ...
  getCategory: async (id: number): Promise<ConcessionCategoryDetail> => {
    const response = await apiClient.get<ConcessionCategoryDetail>(
  ...
  createCategory: async (data: ConcessionCategoryCreate): Promise<ConcessionCategory> => {
    const response = await apiClient.post<ConcessionCategory>(
  ...
  getItem: async (id: number): Promise<ConcessionItemDetail> => {
    const response = await apiClient.get<ConcessionItemDetail>(`/v1/concession-items/${id}/`);
    return response.data;
  },
  ...
  updateItem: async (
    id: number,
    data: Partial<ConcessionItemCreate>
  ): Promise<ConcessionItem> => {
    const response = await apiClient.patch<ConcessionItem>(
      `/v1/concession-items/${id}/`,
      data
    );
    return response.data;
  },
  ...
  deleteVariation: async (id: number): Promise<void> => {
    await apiClient.delete(`/v1/concession-variations/${id}/`);
  },
};
